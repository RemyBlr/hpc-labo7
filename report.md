...
```bash
python3 -c "import random; print(''.join(random.choice('0123456789') for _ in range(50_000_000)), end='')" > ./inputs/pi.txt
```

### Exécution de vérification de correction
...
**Parallélisme potentiellement utile : paquets indépendants.** Si plusieurs dérivations ECG (12 leads dans le fichier de test) doivent être analysées simultanément, chaque lead peut être traité par un thread différent sur son propre `StreamState`. Ce parallélisme est naturel, sans contention, et donnerait un speedup proche de 12× (nombre de leads) sur une machine multi-cœurs. C'est le cas d'usage où OpenMP aurait un intérêt réel ici.
